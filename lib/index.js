...
function getAttributesNodeJSON(context, strict) {
    ...
    try {
        for (var _iterator = Object.keys(context.attributes)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function toObject(xmlStream, schema, objectPath, { strict = false, trimText = true } = {}) {
    const saxStream = _sax2.default.createStream(strict, { xmlns: false });
    const objectStream = new _stream2.default.Readable({ objectMode: true });
...
function toJSON(xmlStream, schema, { strict = false, trimText = true } = {}) {
    const saxStream = _sax2.default.createStream(strict, { xmlns: false });
